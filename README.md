...
图标名称和删除记录会保存到当前匿名用户数据；名称及删除记录另有本地备份。拖动图标调整分类只在本机开发地址启用，尚未固化的调整只写入当前浏览器的 localStorage。当前已确认的 257 枚图标分类已写入源码默认映射，所有新浏览器与线上部署会得到同一布局：基础功能 67、业务类 126、生活服务 30、金融数据类 34。
