...
| 分类维护 | 本地开发环境可拖动图标调整分类；已确认布局固化到源码，额外本机改动发布前必须同步进默认映射 |
...
- 早期已确认：106 枚。
- 最近批次已确认：165 枚。
- 历史已确认总数：271 枚；删除项保留在验收历史中，当前产品底库实际可见 257 枚。
...
- 39 枚已删除基础图标已在 `src/fs-base-icons.tsx` 中做源码级永久排除；新浏览器、新匿名用户和新部署都不得恢复。原始 Iconfont JSON 只作为来源档案保留，不代表产品可见列表。
- 当前产品分类：`全部 / 基础功能 / 业务类 / 生活服务 / 金融数据类`。
- 当前 257 枚可见图标的默认分类数量已固化为：基础功能 67、业务类 126、生活服务 30、金融数据类 34；新浏览器与部署必须保持一致。
...
- 拖动图标调整分类是项目维护者的本机功能，只在 `localhost`、`127.0.0.1` 或 `::1` 启用。拖动产生的增量映射仅写入当前浏览器的本地存储，不进入 Firestore；已确认的产品默认布局另行固化在 `src/App.tsx`，因此新浏览器和部署会得到同一分类。发布本机新调整前，必须把差异同步进源码默认映射。
...
| 分类覆盖 | 是 | 否 | 仅保存尚未固化的本机维护增量；已确认默认分类在源码中，对所有部署生效 |
...
- 不能把“构建通过”写成“用户已确认”。
- 不要恢复已删除图标或按旧静态顺序补位。
- 不要把本机分类覆盖写入 Firestore；确认后的分类应固化进源码默认映射，而不是依赖浏览器状态发布。
